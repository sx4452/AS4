...
for t = 1:T
    ...
    yi = S(i,n);
    Z = Wout*S(i,1:n-1)';
    Wout = (1 - eta*lamda)*Wout + eta*yi/(1+exp(yi*Z))*S(i,1:n-1);
    if t == round(step*T)
        resultX(round(step*10)) = step;
        resultY(round(step*10)) = TestPegasos(Wout, test);
        step = step + 0.1;
